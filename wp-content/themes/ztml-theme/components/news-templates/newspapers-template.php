...
function render_newspapers_template($type_adv = "page", $id = false)
{
	$newspapers = array(
		array(
			'name' => 'Вечерний Минск',
	...
	foreach ($newspapers as $cat) {
		$category = get_term_by('slug', $cat["slug"], 'newspapers');
		if ($id) { ?>
			<div class="container_adv"><?php render_adv($type_adv, $id, 'before_' . $cat["slug"]); ?></div>
<?php }
		render_topic_bar("Читайте «{$cat["name"]}»", true, array(
		...
		render_pdf_attachments(get_posts(
			array(
				'posts_per_page' => 3,
				'post_type' => 'newspaper',
				'post_status' => 'publish',
