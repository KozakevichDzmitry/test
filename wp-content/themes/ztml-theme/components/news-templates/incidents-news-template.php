...
function render_incidents_news_tempalte()
{
?>
	<?php render_topic_bar("Происшествия", true, array(
		'link' => get_site_url() . '/category/proishestviya/',
	...
	<?php $incidents_attached_posts = get_attached_news('3', 'novosti-mira'); ?>
	<?php $incidents_posts = get_default_news('4', 'novosti-mira', $incidents_attached_posts['ids']); ?>

	<div class="box-column-gap incidents-news proishestviya" style="margin-bottom: 30px;">
		<div class="box">
			<?php render_news_template_line($incidents_attached_posts['posts'][0]->ID, true, false); ?>
		</div>
		<div class="box-line-reverse-gap">
			<div class="box">
				<?php render_new_template_image($incidents_attached_posts['posts'][1]->ID); ?>
			</div>
			<div class="box box-list">
				<?php render_news_template_line($incidents_posts[0]->ID, false, true); ?>
				<?php render_news_template_line($incidents_posts[1]->ID, false, true); ?>
			</div>
		</div>
		<div class="box-line-gap">
			<div class="box">
				<?php render_new_template_image($incidents_attached_posts['posts'][2]->ID); ?>
			</div>
			<div class="box box-list">
				<?php render_news_template_line($incidents_posts[2]->ID, false, true); ?>
				<?php render_news_template_line($incidents_posts[3]->ID, false, true); ?>
			</div>
		</div>
	</div>
