...
function render_economy_news_template($is_minimazed = false, $is_district_page = false, $tax_slug = '', $type_adv = "page", $id = false)
{
	if ($id) { ?>
		<div class="container_adv"><?php render_adv($type_adv, $id, 'before_economy_news'); ?></div>
	<?php }
	render_topic_bar("Экономика", true, array(
		'link' => get_site_url() . '/category/ekonomika-2/',
	...
	<div class="box-column-gap economy-news ekonomika-2" style="margin-bottom: 30px;">
		<?php $economy_attached_posts = get_attached_news('1', 'ekonomika-2', $tax_query); ?>
		<?php $economy_posts = get_default_news('2', 'ekonomika-2', $economy_attached_posts['ids'], $tax_query); ?>

		<?php if ($is_minimazed) : ?>
			<div class="box-line-reverse-gap">
				<div class="box">
					<?php render_new_template_image($economy_attached_posts['posts'][0]->ID); ?>
				</div>
				<div class="box box-list">
					<?php render_news_template_line($economy_posts[0]->ID, false, true); ?>
					<?php render_news_template_line($economy_posts[1]->ID, false, true); ?>
				</div>
			</div>
		<?php else : ?>
			<?php $economy_attached_posts = get_attached_news('3', 'ekonomika-2', $tax_query); ?>
			<?php $economy_posts = get_default_news('4', 'ekonomika-2', $economy_attached_posts['ids'], $tax_query); ?>
			...
			<div class="box-line-gap">
				<div class="box">
					<?php render_new_template_image($economy_attached_posts['posts'][1]->ID); ?>
				</div>
				<div class="box box-list">
					<?php render_news_template_line($economy_posts[2]->ID, false, true); ?>
					<?php render_news_template_line($economy_posts[3]->ID, false, true); ?>
				</div>
			</div>
			<div class="box">
				<?php render_news_template_line($economy_attached_posts['posts'][2]->ID, true, false); ?>
			</div>
		<?php endif; ?>
	</div>
<?php
