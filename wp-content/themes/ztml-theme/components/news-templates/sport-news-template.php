...
function render_sport_news_template()
{
?>
	<?php render_topic_bar("Спорт", true, array(
		'link' => get_site_url() . '/category/sport/',
	...
	<?php $sport_attached_posts = get_attached_news('2', 'sport'); ?>
	<?php $sport_posts = get_default_news('2', 'sport', $sport_attached_posts['ids']); ?>

	<div class="box-column-gap sport-news sport" style="margin-bottom: 30px;">
		<div class="box-line-reverse-gap">
			<div class="box">
				<?php render_new_template_image($sport_attached_posts['posts'][0]->ID); ?>
			</div>
			<div class="box box-list">
				<?php render_news_template_line($sport_posts[0]->ID, false, true); ?>
				<?php render_news_template_line($sport_posts[1]->ID, false, true); ?>
			</div>
		</div>
		<div class="box">
			<?php render_news_template_line($sport_attached_posts['posts'][1]->ID, true, true); ?>
		</div>
	</div>
<?php
}
