...
function render_pallete_news_template()
{
?>
	<?php render_topic_bar("Палитра дня", true, array(
		'link' => get_site_url() . '/category/palitra-dnya/',
	...
	<?php $pallete_news_attached_posts = get_attached_news('2', 'palitra-dnya'); ?>
	<?php $pallete_news_posts = get_default_news('2', 'palitra-dnya', $pallete_news_attached_posts['ids']); ?>

	<div class="box-column-gap pallete-news palitra-dnya" style="margin-bottom: 30px;">
		<div class="box-line-gap">
			<div class="box">
				<?php render_new_template_image($pallete_news_attached_posts['posts'][0]->ID); ?>
			</div>
			<div class="box box-list">
				<?php render_news_template_line($pallete_news_posts[0]->ID, false, true); ?>
				<?php render_news_template_line($pallete_news_posts[1]->ID, false, true); ?>
			</div>
		</div>
		<div class="box">
			<?php render_news_template_line($pallete_news_attached_posts['posts'][1]->ID, true, true); ?>
		</div>
	</div>
<?php
}
