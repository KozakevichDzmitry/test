...
function render_culture_news_template()
{
?>
	<?php render_topic_bar("Культура", true, array(
		'link' => get_site_url() . '/category/kultura/',
	...
	<?php $culture_attached_posts = get_attached_news('2', 'kultura'); ?>
	<?php $culture_economy_posts = get_default_news('2', 'kultura', $culture_attached_posts['ids']); ?>


	<div class="box-column-gap culture-news kultura" style="margin-bottom: 30px;">
		<div class="box">
			<?php render_news_template_line($culture_attached_posts['posts'][0]->ID, true, false); ?>
		</div>
		<div class="box-line-reverse-gap">
			<div class="box">
				<?php render_new_template_image($culture_attached_posts['posts'][1]->ID); ?>
			</div>
			<div class="box box-list">
				<?php render_news_template_line($culture_economy_posts[0]->ID, false, true); ?>
				<?php render_news_template_line($culture_economy_posts[1]->ID, false, true); ?>
			</div>
		</div>
	</div>
<?php
